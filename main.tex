...
\begin{itemize}
	...
	\item $\mathcal{O}_{\sf auth}({\sf csk}, \mathbf{c_x}, \cdot)$: On input $\mathbf{z}$, it outputs ${\sf Verify}({\sf Compare}({\sf csk}, \mathbf{c_x}, \mathbf{z}))$.
\end{itemize}
...
Their corresponding oracles for $\mathcal{\tilde O}$ are stateful and memorize all the previous queries. It includes two simulators: $\mathcal{S}_{\sf Enroll}$ and $\mathcal{S}_{\sf Probe}$.
