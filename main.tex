...
\begin{itemize}
	...
	\item {\sf Server}: The server responsible for authenticating the user. It stores the comparison key {\sf csk} and the user's enrollment message $\mathbf{c_x}$ for each domain. On authentication, it compares the probe message with the registered enrollment message and returns the result.  
...
\centerline{
	...
	\begin{algorithm}[H]
	\caption{${\sf Forg}_{\Pi, \mathbb{B}}(\mathcal{A}^\mathcal{O})$}
	\label{alg:forgery_game}
	\begin{algorithmic}[1]
...
\caption{The Forgery Game (Left) and Plain Forgery Game (Right)}
\label{fig:forg_game}
\end{figure}
