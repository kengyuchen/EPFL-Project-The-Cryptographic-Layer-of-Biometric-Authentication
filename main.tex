...
\centering
	...
	\begin{algorithmic}[1]
		...
		\If{$\textsf{Verify}(s) = 1$} \label{alg:reduction-ind-uf-msc1_game:verify}
...
\label{assump:ind-uf-msc:simulator}
	Assume that there exists a simulator $\mathcal{S}$ such that the statistical distance between $\textsf{FE.KeyGen}(\textsf{msk}, \textsf{pp}, \mathbf{r})$, where $(\textsf{msk}, \textsf{pp}) \gets \textsf{FE.Setup}(1^\lambda)$ and $\mathbf{r} \getsdollar \mathbb{F}^{k+2}$, and the output of $\mathcal{S}$ is bounded by $\delta$.
	That is, let $(\textsf{msk}, \textsf{pp}) \gets \textsf{FE.Setup}(1^\lambda)$ and $\mathbf{r} \getsdollar \mathbb{F}^{k+2}$,
	\[
		\Delta(\textsf{FE.KeyGen}(\textsf{msk}, \textsf{pp}, \mathbf{r}), \mathcal{S}) \leq \delta.
	\]
...
\[
	\Pr \left[ \textsf{FE.Dec}(\textsf{FE.KeyGen}( \textsf{msk}, \textsf{pp}, \mathbf{x}), \mathbf{z}) = \textsf{FE.Dec}(\textsf{FE.KeyGen}( \textsf{msk}, \textsf{pp}, \mathbf{x}), \mathbf{z}) \right] \geq 1 - \epsilon
\]
...
In the Device-of-User model, for any distribution family $\mathbb{B}$, if \textsf{FE} is IND secure, then $\forall \mathcal{A}^*$ in the $\textsf{UF-MSC}^*$ game,
	...
	\]
	where $\mathcal{A}^\prime$ is an adversary in the $\textsf{UF}$ game which runs $\mathcal{A}^*$ and simulates $\mathbf{c_x}$ by the output of the simulator $\mathcal{S}$ from Assumption \ref{assump:ind-uf-msc:simulator}.
\end{theorem}
...
\centering
	...
	\begin{algorithmic}[1]
		...
		\If{$\textsf{Verify}(s) = 1$} \label{alg:reduction-ind-uf-msc2_game:verify}
			\State \Return $\tilde{b} = 0$
		\Else
			\State \Return $\tilde{b} \getsdollar \{0, 1\}$
		\EndIf
