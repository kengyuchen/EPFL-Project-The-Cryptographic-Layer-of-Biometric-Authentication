...
\begin{itemize}
	...
	\item $\mathcal{O}_{\textsf{KeyGen}}(\cdot, \cdot)$: On input $(\mathbf{x}^{(0)}, \mathbf{x}^{(1)})$, it outputs $\textsf{FE.KeyGen}(\textsf{msk}, \textsf{pp}, \mathbf{x}^{(b)} )$
...
\begin{theorem}
	...
	For any distribution family $\mathbb{B}$, if \textsf{FE} is IND secure, then $\forall \mathcal{A}$ in the \textsf{UF-MSC} game,
	\[
		\Pr[\textsf{UF-MSC}_{\Pi, \mathbb{B}}(A) \to 1] - \Pr[\textsf{UF-MSC}^*_{\Pi, \mathbb{B}}(A^*) \to 1] = \negl.
	\]
	where $\mathcal{A}^*$ is an adversary in the $\textsf{UF-MSC}^*$ game which runs $\mathcal{A}$ and simulates oracle $\mathcal{O}_{\mathcal{B}}$ by $\mathcal{O}_{\sf samp}(i^*)$ for some index $i^*$ that is never queried by $\mathcal{A}$ in $\mathcal{O}_{\sf samp}$.
...
Given an adversary $\mathcal{A}$ in the \textsf{UF-MSC} game, consider a reduction adversary $\mathcal{R}$ in Algorithm \ref{alg:reduction-ind-uf-msc-star_game} which plays the \textsf{IND} game.
...
\centering
	...
	\centering
	\begin{algorithm}[H]
		\caption{$\mathcal{R}^{\mathcal{O}_{\textsf{KeyGen}}, \mathcal{O}_{\textsf{Enc}}}(\textsf{pp}, \mathcal{A})$}
	\label{alg:reduction-ind-uf-msc-star_game}
	\begin{algorithmic}[1]
		\State $\mathcal{B}^{(0)} \getsdollar \mathbb{B}, \quad \mathbb{B} \gets \mathbb{B} \setminus \mathcal{B}^{(0)}$
...
If the challenge bit $b = 0$, then $\Pr[\textsf{Verify}(s) = 1 \mid b = 0]$ in Algorithm \ref{alg:reduction-ind-uf-msc-star_game} is $\Pr[\textsf{UF-MSC}(\mathcal{A}) \to 1]$.

If the challenge bit $b = 1$, then since $\mathcal{B}^{(1)}$ is never seen by $\mathcal{A}$, and since $\mathcal{B}^{(0)}$ has the same distribution as any $\mathcal{B} \in \mathbb{B}$ in the view of $\mathcal{A}$, $\Pr[\textsf{Verify}(s) = 1 \mid b = 1]$ in Algorithm \ref{alg:reduction-ind-uf-msc-star_game} is $\Pr[ \textsf{UF-MSC}^*(\mathcal{A}^*) \to 1 ]$.
...
\begin{assumption}
	\label{assump:1}
	Assume that there exists a simulator $\mathcal{S}$ such that the statistical distance between $\textsf{FE.KeyGen}(\textsf{msk}, \textsf{pp}, \mathcal{U}_{k+2})$, where $\textsf{msk}, \textsf{pp} \gets \textsf{FE.Setup}(1^\lambda)$ and $\mathcal{U}_{k+2}$ is a uniform distribution over $\mathbb{F}^{k+2}$, and output of $\mathcal{S}$ is bounded by $\delta$.
	\[
		\Delta(\textsf{FE.KeyGen}(\textsf{msk}, \textsf{pp}, \mathcal{U}_{k+2}), \mathcal{S}) \leq \delta.
	\]
...
\begin{assumption}
	\label{assump:2}
Assume that $\forall \mathbf{z}, \forall \mathbf{x} \gets \mathcal{B} \in \mathbb{B}$, two invocations of $\textsf{FE.Dec}(\textsf{FE.KeyGen}( \textsf{msk}, \textsf{pp}, \mathbf{x}), \mathbf{z})$ are the same with a probability bounded below by $p$.
\[
	\Pr[ \textsf{FE.Dec}(\textsf{FE.KeyGen}( \textsf{msk}, \textsf{pp}, \mathbf{x}), \mathbf{z}) = \textsf{FE.Dec}(\textsf{FE.KeyGen}( \textsf{msk}, \textsf{pp}, \mathbf{x}), \mathbf{z})] \geq p
\]
...
Given Assumption \ref{assump:1} with $\delta = \negl$ and Assumption \ref{assump:2} with $p \geq 1 - \negl$.
For any distribution family $\mathbb{B}$, if \textsf{FE} is IND secure, then $\forall \mathcal{A}^*$ in the $\textsf{UF-MSC}^*$ game,
	\[
		\Pr[\textsf{UF-MSC}^*_{\Pi, \mathbb{B}}(A^*) \to 1] - \Pr[\textsf{UF-MSC}^\prime_{\Pi, \mathbb{B}}(A^\prime) \to 1] = \negl.
	\]
	where $\mathcal{A}^\prime$ is an adversary in the $\textsf{UF-MSC}^\prime$ game which runs $\mathcal{A}^*$ and simulates $\mathbf{c_x}$ by the output of the simulator $\mathcal{S}$
\end{theorem}
