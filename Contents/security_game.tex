...
\centering
	...
	\begin{algorithmic}[1]
		...
		\State $\mathbf{c_y}^{(0)} \gets \textsf{Probe}(\textsf{psk}, \mathbf{b}^{(0)})$ \Comment{or $\mathbf{c_y}^{(0)} \gets \mathcal{O}_{\textsf{Probe}}(\mathbf{b}^{(0)})$ }
...
Recall the trivial authentication scheme for any biometric layer we introduced in Section \ref{sec:uf_game}. By Theorem \ref{thm:sEUF-CMA-esk-csk}, we can obtain an authentication scheme $\Pi^\prime$ that is $\textsf{option}$-UF for any $\textsf{option}$ that does not include $\textsf{psk}$.
...
Given any authentication scheme $\Pi$ and a multi-challenge IND-CPA public-key encryption scheme $\textsf{PKE} = (\textsf{PKE.KeyGen}, \allowbreak \textsf{PKE.Enc}, \textsf{PKE.Dec})$, consider the following scheme $\Pi^\prime$. 
