...
\]
for any adversary who has access to $\mathbb{B}$, then $\Pi$ is $\textsf{option}$-UF. 
...
For the IND security, since $\textsf{esk}, \textsf{psk}$ and $\textsf{csk}$ are assumed to be public in this instantiation and should given in $\textsf{option}$, by applying Theorem \ref{thm:ind-tp-fp}, $\Pi$ is not $\textsf{option}$-IND for any \textsf{option} that includes $\mathbf{c_x}$ or $\mathcal{O}_{\mathbf{c_y}}$.
