...
We first reacll the unforgeability \cite{cryptoeprint:2014/394} of a relational hash scheme.
...
For the IND security, we have a negative result for $\Pi$. Note that $\textsf{esk}, \textsf{psk}$ and $\textsf{csk}$ are assumed to be public in this instantiation and should given in $\textsf{option}$.

\begin{theorem}

Let $\textsf{option} = \{\textsf{esk}, \textsf{psk}, \textsf{csk}, \mathbf{c_x}\}$ or $\{\textsf{esk}, \textsf{psk}, \textsf{csk}, \mathcal{O}_{\mathbf{c_y}} \}$. For any distribution family $\mathbb{B}$ that $\textsf{TP} - \textsf{FP} > \frac{1}{\poly}$, and for any relational hash scheme \textsf{RH}, $\Pi$ is not $\textsf{option}$-IND.

\end{theorem}

\begin{proof}

Let $\textsf{option} = \{\textsf{esk}, \textsf{psk}, \textsf{csk}, \mathbf{c_x}\}$, consider the adversary $\mathcal{A}$ in the $\textsf{IND}_{\textsf{option}}$ game in Algorithm \ref{alg:rh:ind}. When the challenge bit $b = 0$, the probability that $\mathcal{A}$ wins is $\textsf{TP}$. When the challenge bit $b = 1$, the probability that $\mathcal{A}$ wins is $1 - \textsf{FP}$. Now, 
...
\centering
	...
	\begin{algorithm}[H]
	\caption{$\mathcal{A}^{\mathcal{O}_{\mathcal{B}^{(0)}}, \mathcal{O}_{\mathcal{B}^{(1)}}} (\textsf{esk} = \textsf{psk} = \textsf{csk} = \textsf{pk}, \mathbf{c_x} )$}
	\label{alg:rh:ind}
	\begin{algorithmic}[1]
...
The case when $\textsf{option} = \{\textsf{esk}, \textsf{psk}, \textsf{csk}, \mathcal{O}_{\mathbf{c_y}} \}$ can be analyzed similarly by considering an adversary who first asks a $\mathbf{c_y}$ and verifies its relation with $\mathbf{h_x}^{(0)}$, which is created from $\mathcal{O}_{\mathcal{B}^{(0)}}$.

\end{proof}

We note that this insecurity result holds whenever $\textsf{csk}$ and at least one of $\textsf{esk}$ and $\textsf{psk}$ are public. We write this observation formally in the following theorem.

\begin{theorem}

Given any distribution family $\mathbb{B}$ that $\textsf{TP} - \textsf{FP} > \frac{1}{\poly}$. $\Pi$ is not $\textsf{option}$-IND for an $\textsf{option}$ that includes $\textsf{csk}$ and at least one of $\textsf{esk}$ and $\textsf{psk}$.
...
With the same argument in Theorem \ref{thm:rh:ind:particular-biometri-layer}, one can prove that this new scheme is now $\{\textsf{csk}, \mathbf{c_x}, \mathcal{O}_{\mathbf{c_y}}\}$-IND, albeit at the cost of requiring $\textsf{esk}$ and $\textsf{psk}$ to remain secret.
