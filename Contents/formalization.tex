...
Ideally, we hope $\textsf{TP}$ to be $1$ and $\textsf{FP}$ to be negligible. However, due to the inherent nature of biometrics, there might be a nonzero false negative rate $ 1 - \textsf{TP} > 0$ and a $\textsf{FP}$ that is not negligible. Our security model and analysis also take these possibilities into consideration.
