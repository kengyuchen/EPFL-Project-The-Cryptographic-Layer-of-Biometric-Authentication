...
% Scanner and Device
\draw [arrow]  ($(device.west) + (-1.3, -1.5)$) -- node[align=center, above] {{\sf query} \\ $\mathcal{O}_\mathcal{B}$} ($(scanner.east) + (0.7, -1.5)$);
\draw [arrow]  ($(scanner.east) + (0.7, -1.7)$) -- node[align=center, below] {{\sf resp.} \\ $\mathbf{b} \gets \mathcal{B}$} ($(device.west) + (-1.3, -1.7)$);
